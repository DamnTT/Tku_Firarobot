...
# 障礙物(敵方機器人)
    ...
    robot1 = {'state': '', 'ball_is_handled': False,
              'ball_dis': 0, 'position': {'x': 0, 'y': 0, 'yaw': 0}}
    robot2 = {'state': '', 'ball_is_handled': False,
              'ball_dis': 0, 'position': {'x': 0, 'y': 0, 'yaw': 0}}
    robot3 = {'state': '', 'ball_is_handled': False,
              'ball_dis': 0, 'position': {'x': 0, 'y': 0, 'yaw': 0}}
    near_robot = {'state': '', 'ball_is_handled': False,
                  'ball_dis': 0, 'position': {'x': 0, 'y': 0, 'yaw': 0}}
    near_robot_ns = ""
    r1_role = ""
    r2_role = ""
    ...
    def __init__(self, sim=False):
        ...
        rospy.Subscriber(
            POSITION_TOPIC, PoseWithCovarianceStamped, self._GetPosition)
        rospy.Subscriber('BlackRealDis', Int32MultiArray,
                         self._GetBlackItemInfo)
        ...
        robot2_sub = message_filters.Subscriber(
            '/robot2/strategy/state', RobotState)
        robot3_sub = message_filters.Subscriber(
            '/robot3/strategy/state', RobotState)
        ts = message_filters.ApproximateTimeSynchronizer(
            [robot2_sub, robot3_sub], 10, 0.1, allow_headerless=True)
        ts.registerCallback(self.MulticastReceiver)
        s = rospy.Service('passing_action', Trigger, self._PassingServer)

        if not sim:
            rospy.Subscriber('interface/Two_point',
                             Two_point, self._GetTwopoint)
            rospy.Subscriber(IMU, inertia, self._GetImu)
            self.RobotBallHandle = self.RealBallHandle
        else:
            self.sim_hold_pub = rospy.Publisher(
                'motion/hold', Bool, queue_size=1)
...
# ------------------------------------------------------------------------
#   Adjust robot arguement
# ------------------------------------------------------------------------

    def TuningVelocityContorller(self, p, i, d, cp=Cp_v):  # PID速度控制器
        self.pid_v.setpoint = cp
        self.pid_v.tunings = (p, i, d)

    def TuningAngularVelocityContorller(self, p, i, d, cp=Cp_w):  # PID角速度控制器
        self.pid_w.setpoint = cp
        self.pid_w.tunings = (p, i, d)

    def ChangeVelocityRange(self, m, M):   # 控制速度在最大速度和做小速度範圍之間
        self.__minimum_v = m
        self.pid_v.output_limits = (-1*M, M)

    def ChangeAngularVelocityRange(self, m, M):  # 控制角速度在最大速度和做小速度範圍之間
        self.__minimum_w = m
        self.pid_w.output_limits = (-1*M, M)

    def ChangeBallhandleCondition(self, dis, ang):   # 改變持球條件 當球在指定距離和角度之內 判斷為持球
...
# ------------------------------------------------------------------------
    ...
    def RobotStatePub(self, state):  # 機器人狀態發佈ROS
        m = RobotState()
        m.state = state
    ...
    def ConvertSpeedToPWM(self, x, y):  # PWM
        reducer = 24
        max_rpm = 7580
        wheel_radius = 0.11
...
# 輸出移動控制
    ...
    def RobotCtrlS(self, x, y, yaw, pass_through=False):
        if pass_through:
            ...
            msg = Twist()
            output_x, output_y = self.Rotate(
                unit_vector[0]*output_v, unit_vector[1]*output_v, ROTATE_V_ANG)
...
# ------------------------------------------------------------------------
#   Ros2 function with another robot
# ------------------------------------------------------------------------

    def MulticastReceiver(self, r2_data, r3_data):  # 接收其他兩台機器人的資料
    ...
    def GetState(self, robot_ns):   # 接收其他機器人狀態
    ...
    def SetMyRole(self, role):  # 獲取自己是第幾台機器人
        if "robot1" in rospy.get_namespace():
            self.r1_role = role
        elif "robot2" in rospy.get_namespace():
            self.r2_role = role
...
# ------------------------------------------------------------------------
    ...
    def _GetTwopoint(self, vision):  # 球門
    ...
    def _GetBlackItemInfo(self, vision):  # 掃描線
        self.__obstacle_info['ranges'] = vision.data

    def _GetImu(self, imu_3d):  # imu
        front_ang = math.degrees(imu_3d.yaw) + 90
        # caculate front angle by imu
        self.__robot_info['imu_3d']['yaw'] = imu_3d.yaw

    def _GetPosition(self, loc):   # 定位
